...
The error cab happen during run time for any reason. The person forgets to plug in the microntroller for debuggng purposes. Another issues would be the micro controller isn't on.
...
'''
The makefig function produces three plots that read telemetry data from global arrays. 
'''
def makefig():
...
'''
The get data is responsible for retrieving the data from the serial connection. Saves incoming data in a excel file. Plots the realtime data as well.'''
    ...
    while(x==0):  ## Read data infinitley until the disconnect button is pressed.
        ...
        try:       
            ...
            if (filter_data):
                ...
                # Display variables on the screen in text and ploting as well
                # Data Label 1                               temp1
                v=StringVar()                            
...
if __name__ == "__main__":
    ...
    baudrate=Entry(width=7)
    baudrate.place(x=1200,y=420)
